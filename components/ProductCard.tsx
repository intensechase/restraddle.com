...
export default function ProductCard({ product }: { product: Product }) {
    ...
    <>
      <div className="flex aspect-square items-center justify-center bg-[radial-gradient(circle_at_50%_30%,#33201d,var(--surface-2))]">
        <svg viewBox="0 0 100 100" className="h-20 w-20" fill="none">
          ...
          />
          <text x="50" y="60" textAnchor="middle" fontSize="26" fill="var(--gold)">
            {MARK[product.category]}
          </text>
        </svg>
      </div>
      <div className="p-4">
        <div className="text-[10px] font-bold uppercase tracking-wide text-gold">{product.categoryLabel}</div>
  ...
  return (
    <div className="overflow-hidden rounded-lg border-2 border-border bg-surface transition-colors hover:border-gold">
      {product.amazonUrl ? (
