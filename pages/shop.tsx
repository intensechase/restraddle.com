...
import { CATEGORY_COLOR, PRODUCTS, type ProductCategory } from "@/data/products";
    ...
    <div className="min-h-screen">
      ...
      <div className="mx-auto max-w-6xl px-7">
        <div className="flex flex-wrap gap-2.5 border-b-2 border-dashed border-border py-5">
          {FILTERS.map((f) => {
            const color = f.value === "all" ? "var(--red-bright)" : CATEGORY_COLOR[f.value];
            return (
              <button
                key={f.value}
                onClick={() => setActive(f.value)}
                className="rounded-full border-2 px-4.5 py-2 text-[12.5px] font-bold uppercase tracking-wide transition-colors"
                style={
                  active === f.value
                    ? { borderColor: color, color, backgroundColor: "var(--surface-2)" }
                    : { borderColor: "var(--border)", backgroundColor: "var(--surface)", color: "var(--ink-mute)" }
                }
              >
                {f.label}
              </button>
